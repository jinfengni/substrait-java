...
public class SubstraitRelNodeConverter extends AbstractRelVisitor<RelNode, RuntimeException> {
  ...
  public RelNode visit(Aggregate aggregate) throws RuntimeException {
    ...
    RelBuilder.GroupKey groupKey =
        relBuilder.groupKey(
            aggregate.getGroupings().get(0).getExpressions().stream()
  ...
  private ImmutableBitSet fromGrouping(List<Expression> expressions) {
    List<RexNode> groupExprs =
        expressions.stream().map(expr -> expr.accept(expressionRexConverter)).toList();
    List<Integer> bitIndex =
        groupExprs.stream()
            .filter(e -> e instanceof RexInputRef)
            .map(e -> (RexInputRef) e)
            .map(e -> e.getIndex())
            .toList();
    return ImmutableBitSet.of(bitIndex);
  }
